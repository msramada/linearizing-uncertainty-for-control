...
B = randn(liftedDim, 1)
function loss_fn(p, ls, U, states, states⁺, A, B)
	...
	TransformLoss = 1 * mean((states .- states_from_gₓ) .^ 2) +
				1 * mean((states⁺ .- states_from_gₓ₊) .^ 2)
					
	PredictionLoss = 1 * mean((Kgₓ .- gₓ₊) .^ 2) + 
					1 * mean((states_from_Kgₓ .- states⁺) .^ 2)
...
for epoch=1:N_EPOCHS
	...
	U = U_rec[:,indices]
	Loss = loss_fn(ps, ls, U, πₖ, πₖ⁺, A, B)
	avg_loss += Loss / N_EPOCHS
	grad  = Zygote.gradient(p -> loss_fn(p, ls, U, πₖ, πₖ⁺, A, B), ps)[1]
