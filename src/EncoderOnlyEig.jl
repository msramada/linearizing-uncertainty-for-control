...
Λ = Diagonal(rand(liftedDim,))
Λ = Diagonal(0.8 .+ (1 .- rand(liftedDim,)) .* 0.2)
B = randn(liftedDim, 1)
function loss_fn(p, ls, states_list, controls_list, Λ, B)
	pred_horizon = length(states_list)
	states = states_list[1]
	gₓ, _ = Φₑ(states, p, ls)
	Kgₓ = [Λ B] * [gₓ;controls_list[1]]
	gₓ₊, _ = Φₑ(states_list[2], p, ls)	
	PredictionLoss = 1 * sum((Kgₓ .- gₓ₊) .^2) + 
					0 * sum((Kgₓ[1:NinfoState,:] .- states_list[2]).^2)
	#=
...
LS_N = 1000
function Train(ps, opt_state, Λ, B)
avg_loss = 0.0
bb = η
α = 0.0
for epoch=1:N_EPOCHS
	...
	Loss, Back, = Zygote.pullback(p -> loss_fn(p, 
							ls, states_list, controls_list, Λ, B), ps)
	grad, = Back(1.0)
...
if epoch % 1000 == 0
	...
	xTrunc0 = inv(S) * X[:,1]
	X = inv(S) * X
	PredFeaturesDeep = ls_lsim(Λ, B, 0, [U_rec 0], feature0=xTrunc0)
...
ps, Ann, Bnn = Train(params, opt_state, Λ, B)
